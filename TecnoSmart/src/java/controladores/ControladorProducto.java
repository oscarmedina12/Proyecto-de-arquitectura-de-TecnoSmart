         ...
         try{
            ...
            if(codigo<1||tipoProducto.equals("")||modeloProducto.equals("")||descripcionProblema.equals("")||precio<1||
                ...
                if(pd.obtenerProducto(nuevoProducto.getCodigo())==null){
                    response.sendRedirect("Listar.jsp?msj=...");
                }else{
                   int respuesta = pd.modificar(nuevoProducto);
                   if(respuesta>0){
                       response.sendRedirect("Listar.jsp?msj=Estado de Reparacion modificada");
                   }else{
                       response.sendRedirect("Listar.jsp?msj=El estado no se pudo modificar");
                   }
                }
            }
         }catch(Exception e){
